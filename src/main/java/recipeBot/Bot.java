...
public class Bot extends TelegramLongPollingBot {
    ...
    public void handleCallback(CallbackQuery callbackQuery){
        ...
        if(data.startsWith("EDITFIELD_")) {
            ...
            switch(field) {
                case "NAME":
                    ...
                    replaceMessageWithTextAndAddCancel(id, callbackQuery.getMessage().getMessageId(), "Enter new name for "+recipeName+":");
                    break;
                case "CATEGORY":
                ...
                case "DESCRIPTION":
                    ...
                    replaceMessageWithTextAndAddCancel(id, callbackQuery.getMessage().getMessageId(), "Enter new description for "+recipeName+":");
                    break;
                case "INGREDIENTS":
                    userState.put(id, State.EDITING_INGREDIENTS);
                    replaceMessageWithTextAndAddCancel(id, callbackQuery.getMessage().getMessageId(), "Enter new ingredients for "+recipeName+" (separated by lines):");
                    break;
                case "INSTRUCTIONS":
                    userState.put(id, State.EDITING_INSTRUCTIONS);
                    replaceMessageWithTextAndAddCancel(id, callbackQuery.getMessage().getMessageId(), "Enter new instructions for "+recipeName+" (separated by lines):");
                    break;
            }
            return;
        }
        ...
        if(data.equals("IMPORT_URL")){
            replaceMessageWithTextAndAddCancel(id, callbackQuery.getMessage().getMessageId(), "Please send the URL of the recipe you want to import:");
            userState.put(id, State.WAITING_FOR_URL); 
            return;
        }

        if(data.equals("IMPORT_IMAGE")){
            replaceMessageWithTextAndAddCancel(id, callbackQuery.getMessage().getMessageId(), "Please send the image of the recipe you want to import:");
